...
//            Logger::write('当前数据库配置文件为db_test','dbconfig');
    ...
    private function insert($table,$data=array())
    {
        ...
        try{
            ...
            if(!$pdo_stat)
            {
                Logger::sql($sql,$params,__METHOD__);
                return false;
            }
            $pdo_stat->execute($params);
    ...
    private  function getRow($table,$where='',$w_params=array(),$columns='*')
    {
        ...
        try{
            ...
            if(!$pdo_stat)
            {
                Logger::sql($sql,$w_params,__METHOD__,"ERROR");
                return false;
            }
    ...
    private function decr($table,$where,$w_params,$field,$num=1)
    {
        $sql="update `{$table}` set `{$field}` = `{$field}` - {$num} where {$where}";
        try{
            $pdo_stat = $this->db($this->db)->prepare($sql);
    ...
    public function __call($name, array $arguments)
    {
        ...
        while($i--)
        {
            ...
            {
                if($i<2)
                    Logger::write("恢复数据库操作 $name",__METHOD__,"INFO");
                break;
            }
    ...
    {
        try{
            return $this->db($this->db)->exec($sql);
        }catch (PDOException $e)
        {
