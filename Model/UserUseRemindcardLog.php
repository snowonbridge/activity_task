...
class UserUseRemindcardLog extends Model{
    public $db = 'activity';
    public $table = 'user_use_remindcard_log';
    ...
    public function getSetNum($param,$month_time='',$is_get=0)
    {
        ...
        $num = ATCode::getCache(Okey::UserRemindcardNum($uid,$activity_id,$month_time),function($uid,$activity_id,$month_time){
        ...
        if($param['from_num'] == 0)
            return $num;
        ATCode::rmCache(Okey::UserRemindcardNum($uid,$activity_id,$month_time));
        if(!$num)
        {
            $data['uid'] = $param['uid'];
