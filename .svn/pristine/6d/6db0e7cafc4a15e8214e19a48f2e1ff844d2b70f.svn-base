...
class UserEverydayActivityCountsLog extends Model{
    public $db = 'activity';
    public $table = 'user_everyday_activity_counts_log';
    ...
    public function getAchieveCounts($uid,$activity_id,$add_time='')
    {
        $add_time = $add_time?$add_time:date('Ymd',time());
        $r = $this->getRow($this->table,'uid=:uid and activity_id=:activity_id and add_time=:add_time',[':uid'=>$uid,':activity_id'=>$activity_id,':add_time'=>$add_time]);

        return isset($r['achieve_counts'])?(int)$r['achieve_counts']:0;
    }
