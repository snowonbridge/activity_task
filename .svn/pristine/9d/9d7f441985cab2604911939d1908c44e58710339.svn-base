...
class UserPurchaseDiamandLog extends Model{
    public $db = 'activity';
    public $table = 'user_purchase_diamand_log';
    ...
    public function getSetNum($param,$month_time='',$is_get=0)
    {
        ...
        $num = ATCode::getCache(Okey::UserDiamondNum($uid,$activity_id,$month_time),function($uid,$activity_id,$month_time){
        ...
        ATCode::rmCache(Okey::UserDiamondNum($uid,$activity_id,$month_time));
        if(!$num)
        {
            $data['uid'] = $param['uid'];
