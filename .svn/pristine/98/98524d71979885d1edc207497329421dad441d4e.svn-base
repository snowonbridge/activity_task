<?php
/**
 * Created by PhpStorm.
 * User: nihao
 * Date: 17-9-9
 * Time: 上午10:03
 */

namespace Workerman\Model;


use Workerman\Lib\Model;

class UserOnlineLog extends Model{
    public $db = 'activity';
    public $table = 'user_online_log';

    //领取状态@0禁止,1可领取,2未领取
    const RECEIVE_FORBID = 0;
    const RECEIVE_YES= 1;
    const RECEIVE_NO = 2;

    public function rule()
    {
        /**
         * `uid` int(11) NOT NULL DEFAULT '0',
        `activity_id` int(11) NOT NULL DEFAULT '0' COMMENT '活动id',
        `challege_list` varchar(1024) NOT NULL DEFAULT '' COMMENT '挑战项目列表',
        `gift_list` varchar(504) NOT NULL DEFAULT '' COMMENT '奖励列表',
        `is_receive` tinyint(4) NOT NULL DEFAULT '0' COMMENT '领取状态@0禁止,1可领取,2未领取',
        `add_time` int(11) NOT NULL DEFAULT '0' COMMENT '添加时间@格式:年月日',
         * 好友前置条件@0无,1与两个好友一起玩,2自己开房,3不要自己开房
         */
        return [
            ['uid','required'],
            ['activity_id','required'],
            ['challege_list','required'],
            ['gift_list','required'],
            ['is_receive','required'],
            ['frequency','required'],
            ['img_icon','required'],
            ['current_frequency','required'],
        ];
    }

    /**
     * @desc 增加每日任务记录
     * @param array $params
     * @return bool
     */
    public function addEveryTask($params=array())
    {
        if(!$this->validate($params,$this->rules()))
        {
            return false;
        }
        $params['add_time'] = date('Ymd',time());
        $params['create_time'] = time();
        $params['update_time'] = time();
        return  $this->insert($this->table,$params);

    }
    /**
     * @desc 增加天天福利记录
     * @param array $params
     * @return bool
     */
    public function addDayWeal($params=array())
    {
        if(!$this->validate($params,$this->rules()))
        {
            return false;
        }
        $params['create_time'] = time();
        $params['update_time'] = time();
        return  $this->insert($this->table,$params);

    }

    /**
     * @desc 获取每日任务的最后一行,因为一天有多条记录.注意重置时间是1天
     * @param $uid
     * @param $activity_id
     * @param string $add_time
     * @return mixed
     */
    public function getEveryDayOne($uid,$activity_id,$add_time='')
    {
        $add_time = !$add_time?date('Ymd',time()):$add_time;
        $r = $this->getRow($this->table,'uid=:uid and activity_id=:activity_id and add_time=:add_time order by add_time desc ',[
            ':uid'=>$uid,':activity_id'=>$activity_id,':add_time'=>$add_time
        ]);
        return $r;
    }

    /**
     * @desc 获取关卡每日重置任务的状态
     * @param $uid
     */
    public function getEveryDayAll($uid,$activity_ids=array())
    {
        if(!$activity_ids)
            return false;
        $add_time = date('Ymd',time());
        $r = $this->getRows($this->table,'uid=:uid and activity_id in ('.implode(',',$activity_ids).') and add_time=:add_time group by activity_id',
            [ ':uid'=>$uid,':add_time'=>$add_time]);
        return $r;
    }


    /**
     * @desc 更新活动进度状态
     * @param $id
     * @param $data
     * @return bool
     */
    public function updateStatus($id,$data)
    {
        return $this->update($this->table,$data,'id=:id',[':id'=>$id]);
    }

    public function receiveEveryDayGift($uid,$activity_id,$add_time='')
    {
        $add_time = !$add_time?date('Ymd',time()):$add_time;
        $r = $this->update($this->table,['is_receive'=>self::RECEIVE_YES],'uid=:uid and activity_id=:activity_id and add_time=:add_time',[
            ':uid'=>$uid,':activity_id'=>$activity_id,':add_time'=>$add_time]);
        return $r;
    }

    /**
     * @desc
     * @param $uid
     * @param $activity_id
     */
    public function validFrequency($uid,$activity_id)
    {
        $add_time = date('Ymd',time());
        $frequency = $this->getRow($this->table,'uid=:uid and activity_id=:activity_id and add_time=:add_time order by add_time desc',[
            ':uid'=>$uid,':activity_id'=>$activity_id,':add_time'=>$add_time]);
        $base_frequency = $frequency['frequency'];
        $currentmax_frequency = $frequency['current_frequency'];
        if($currentmax_frequency >=$base_frequency)
            return false;
        return true;
    }
    public function getUserFrequency($uid,$activity_id)
    {
        $add_time = date('Ymd',time());
        $frequency = $this->getRow($this->table,'uid=:uid and activity_id=:activity_id and add_time=:add_time order by add_time desc',[
            ':uid'=>$uid,':activity_id'=>$activity_id,':add_time'=>$add_time]);

        return isset($frequency['current_frequency'])?intval($frequency['current_frequency']):0;
    }
    /**
     * @desc 是否存在禁止领取或未领取的任务
     * @param $uid
     * @param $activity_id
     * @return int
     */
    public function hasNoReceived($uid,$activity_id)
    {
        $add_time = date('Ymd',time());
        $r = $this->count($this->table,'uid=:uid and activity_id=:activity_id and add_time=:add_time and is_receive in (0,2) order by add_time desc',[
            ':uid'=>$uid,':activity_id'=>$activity_id,':add_time'=>$add_time]);

        return $r?1:0;
    }
} 