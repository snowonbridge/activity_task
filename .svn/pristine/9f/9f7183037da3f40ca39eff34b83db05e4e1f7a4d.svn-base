...
class UserUseRoomcardLog extends Model{
    public $db = 'activity';
    public $table = 'user_use_roomcard_log';
    ...
    public function getSetNum($param,$month_time='',$is_get=0)
    {
        ...
        $num = ATCode::getCache(Okey::UserRoomcardNum($uid,$activity_id,$month_time),function($uid,$activity_id,$month_time){
        ...
        if($param['from_num'] == 0)
            return $num;
        ATCode::rmCache(Okey::UserRoomcardNum($uid,$activity_id,$month_time));
        if(false === $num)
        {
            $data['uid'] = $param['uid'];
...
//            Redis::getIns()->incrBy(Okey::UserExchangeLog($uid,$activity_id,$month_time),$param['from_num']);
